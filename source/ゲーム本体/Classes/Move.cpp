...
bool Move::onTouchBegan(Touch* touch, Event* event) {
	...
	for(int i=0; i<5; i++){
		...
		if(rect.containsPoint(touch->getLocation())){
			...
			if(skillButton[i].getButton()->getOpacity() == 128) {
				skillButton[i].getButton()->setOpacity(255);
			}
			else {
				skillButton[i].getButton()->setOpacity(128);
			}
		}
	}
...
void Move::update(float delta) {
	...
	// 敵の攻撃予兆部分テスト
	if(count <= 0) {
		as.create(Vec2(sprite->getPositionX(), sprite->getPositionY()), 180);
		count = 120;
	}
	else {
		count--;
